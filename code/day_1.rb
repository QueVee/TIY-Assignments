...
# write the shortest possible code that will print out the string 
"Hello! Isn't it #{weather} today?"
...
"3 + 2 is #{3+2}" #=> "3 + 2 is 5"
#first the #{3+2} calculates to 5, then the string is printed with the answer.
...
#The answer to 5>4 is true, so comes back 'greater' which is added to the string.

# Add a comment after the following code block showing its return value. 
# Then add a second comment explaining in English why the code does what it does.
"Five is #{5 > 6 ? 'greater' : 'less'} than six." #=> "Five is less than six."
#5 is not greater than 6, so the false='less' is added to the string.
...
#The double = sign means 'the same as' which 5 is not of 100.  the false statement of 'not ' is returned and added to the string.
...
#The |n| pipes the array in and out to n*3, calculating the new array numbers.

# beginning with a list of the numbers 3, -2, 7.5, and 90, make a list containing their absolute values.
array = [3, -2, 7.5, 90].map {|n| n.abs}
=> [3, 2, 7.5, 90]
...
%w(Joe, Kelly, Spencer, Peter, Ivan, Andell, Angela, Jeff)
